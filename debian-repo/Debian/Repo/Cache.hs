...
import Debian.Repo.Types (AptCache(aptArch, aptBaseSliceList, aptBinaryPackages, aptReleaseName, aptSourcePackages, globalCacheDir), BinaryPackage(packageID), binaryPackageName, EnvRoot(EnvRoot), PackageID(packageVersion), PackageIndex(..), Release(releaseName), SourcePackage(sourcePackageID), sourcePackageName)
