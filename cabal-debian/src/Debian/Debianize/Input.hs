...
inputAtoms debian name xs =
    case (BinPkgName (dropExtension name), takeExtension name) of
      ...
      (p, ".postrm") ->    readFile (debian </> name) >>= \ text -> return $ insertAtom (Just p) (DHPostRm text) xs
      (p, ".preinst") ->   readFile (debian </> name) >>= \ text -> return $ insertAtom (Just p) (DHPreInst text) xs
      (p, ".prerm") ->     readFile (debian </> name) >>= \ text -> return $ insertAtom (Just p) (DHPreRm text) xs
