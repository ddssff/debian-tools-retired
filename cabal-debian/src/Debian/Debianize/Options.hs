...
options =
    [ Option "v" ["verbose"] (ReqArg (\ s atoms -> execDeb (verbosity (read' (\ s' -> error $ "verbose: " ++ show s') s)) atoms) "n")
      ...
      Option "n" ["dry-run", "compare"] (NoArg (\ atoms -> execDeb (dryRun True) atoms))
             "Just compare the existing debianization to the one we would generate.",
      Option "h?" ["help"] (NoArg (\ atoms -> execDeb (debAction Usage) atoms))
             "Show this help text",
      Option "" ["executable"] (ReqArg (\ path x -> executableOption path (\ bin e -> execDeb (doExecutable bin e) x)) "SOURCEPATH or SOURCEPATH:DESTDIR")
      ...
      Option "" ["ghc-version"] (ReqArg (\ ver x -> execDeb (compilerVersion (const (Just (last (map fst (readP_to_S parseVersion ver)))))) x) "VERSION")
      ...
      Option "" ["disable-haddock"] (NoArg (execDeb (noDocumentationLibrary True)))
             (unlines [ "Don't generate API documentation packages, usually named"
                      ...
                      , "haddock bug."]),
      Option "" ["missing-dependency"] (ReqArg (\ name atoms -> execDeb (missingDependency (BinPkgName name)) atoms) "DEB")
             (unlines [ "This is the counterpart to --disable-haddock.  It prevents a package"
                      ...
                      , "build dependency."]),
      Option "" ["source-package-name"] (ReqArg (\ name x -> execDeb (sourcePackageName (SrcPkgName name)) x) "NAME")
      ...
      Option "" ["disable-library-profiling"] (NoArg (execDeb (noProfilingLibrary True)))
      ...
      Option "" ["maintainer"] (ReqArg (\ maint x -> execDeb (either (error ("Invalid maintainer string: " ++ show maint)) maintainer (parseMaintainer maint)) x) "Maintainer Name <email addr>")
      ...
      Option "" ["build-dep"]
                 (ReqArg (\ name atoms ->
                              case parseRelations name of
                                ...
                                Right rss -> execDeb (buildDeps rss) atoms) "Debian package relations")
      ...
      Option "" ["build-dep-indep"]
                 (ReqArg (\ name atoms ->
                              case parseRelations name of
                                ...
                                Right rss -> execDeb (buildDepsIndep rss) atoms) "Debian binary package name")
                 (unlines [ "Similar to --build-dep, but the dependencies are added to Build-Depends-Indep, e.g.:"
                          , ""
                          , "    --build-dep-indep perl" ]),
      Option "" ["dev-dep"] (ReqArg (\ name atoms -> execDeb (extraDevDeps (Rel (BinPkgName name) Nothing Nothing)) atoms) "Debian binary package name")
      ...
      Option "" ["depends"]
             (ReqArg (\ arg atoms -> execDeb (mapM_ (uncurry depends) (parseDeps arg)) atoms) "deb:deb,deb:deb,...")
      ...
      Option "" ["conflicts"]
             (ReqArg (\ arg atoms -> execDeb (mapM_ (uncurry conflicts) (parseDeps arg)) atoms) "deb:deb,deb:deb,...")
             "Like --depends, modifies the Conflicts field.",
      Option "" ["replaces"]
             (ReqArg (\ arg atoms -> execDeb (mapM_ (uncurry replaces) (parseDeps arg)) atoms) "deb:deb,deb:deb,...")
             "Like --depends, modifies the Replaces field.",
      Option "" ["provides"]
             (ReqArg (\ arg atoms -> execDeb (mapM_ (uncurry provides) (parseDeps arg)) atoms) "deb:deb,deb:deb,...")
             "Like --depends, modifies the Provides field.",
      Option "" ["map-dep"] (ReqArg (\ pair atoms -> case break (== '=') pair of
                                                       (cab, (_ : deb)) -> execDeb (extraLibMap cab (rels deb)) atoms
      ...
      Option "" ["deb-version"] (ReqArg (\ version atoms -> execDeb (debVersion (parseDebianVersion version)) atoms) "VERSION")
             "Specify the version number for the debian package.  This will pin the version and should be considered dangerous.",
      Option "" ["revision"] (ReqArg (\ rev atoms -> execDeb (revision (Just rev)) atoms) "REVISION")
      ...
      Option "" ["epoch-map"] (ReqArg (\ pair atoms -> case break (== '=') pair of
                                                         (_, (_ : ['0'])) -> atoms
                                                         (cab, (_ : [d])) | isDigit d -> execDeb (epochMap (PackageName cab) (ord d - ord '0')) atoms
      ...
      Option "" ["exec-map"] (ReqArg (\ s atoms -> case break (== '=') s of
                                                     (cab, (_ : deb)) -> execDeb (execMap cab (rels deb)) atoms
      ...
      Option "" ["omit-lt-deps"] (NoArg (execDeb (omitLTDeps True)))
             (unlines [ "Remove all less-than dependencies from the generated control file.  Less-than"
                      ...
                      , "google 'cabal hell'."]),
      Option "" ["quilt"] (NoArg (execDeb (sourceFormat Quilt3)))
             "The package has an upstream tarball, write '3.0 (quilt)' into source/format.",
      Option "" ["builddir"] (ReqArg (\ s atoms -> execDeb (buildDir (s </> "build")) atoms) "PATH")
      ...
      Option "f" ["flags"] (ReqArg (\ fs atoms -> execDeb (mapM_ (uncurry cabalFlagAssignment) (flagList fs)) atoms) "FLAGS")
      ...
      Option "" ["debianize"] (NoArg (\ atoms -> execDeb (debAction Debianize) atoms))
             "Deprecated - formerly used to get what is now the normal benavior.",
      Option "" ["substvar"] (ReqArg (\ name atoms -> execDeb (debAction (SubstVar (read' (\ s -> error $ "substvar: " ++ show s) name))) atoms) "Doc, Prof, or Dev")
