...
test6 =
    TestLabel "test6" $
    TestCase ( do old@(Debianization {changelog = log}) <- inputDebianization "test-data/artvaluereport2/output"
...
-}
                             ...
                             flip (foldr (\ s deb -> Atom.depends (BinPkgName "artvaluereport2-server") (anyrel (BinPkgName s)) deb)) ["libjs-jquery", "libjs-jquery-ui", "libjs-jcrop", "texlive-latex-recommended", "texlive-latex-extra", "texlive-fonts-recommended", "texlive-fonts-extra", "libjpeg-progs", "netpbm"] $
...
-}
      ...
      hostname' = "appraisalreportonline.com"
      portNum :: BinPkgName -> Int
      portNum (BinPkgName deb) =
          case deb of
